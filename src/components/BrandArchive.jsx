...
export default function BrandArchive({ onPlayVideo }) {
  const [activeTab, setActiveTab] = useState('food');

  const currentCategory = categories.find((c) => c.id === activeTab) || categories[0];

  return (
    <section id="brands" className="max-w-6xl mx-auto px-8 py-24 border-t border-border relative">
      {/* 背景點綴網格 */}
      <div className="absolute inset-0 grid-bg opacity-10 pointer-events-none" />

      {/* 區段標頭 */}
      <div className="mb-16 relative z-10">
        <div className="mono text-xs text-aurora-blue mb-4 uppercase tracking-[0.3em]">
          04 // Collaboration Matrix
        </div>
        <h2 className="text-4xl md:text-6xl font-black tracking-tighter uppercase text-white glow-title">
          Client <span className="text-aurora-blue">Portfolio</span>
        </h2>
        <p className="text-zinc-500 font-light mt-4 max-w-xl text-sm leading-relaxed">
          歷年參與的代表性品牌與品相，涵蓋食品飲料、科技家電、汽車交通、美妝保健及金融保險等多重領域。
        </p>
      </div>
